...
## 6、垃圾标记算法
现在的GC都是采用了分代收集算法来回收垃圾的，Java堆作为GC主要管理的区域，被细分为新生代和老年代。新生代又可以划分为Eden空间、From Survivor空间、To Survivor空间等。

### 6.1 Java中的引用
Java将引用分为强引用、软引用、弱引用和虚引用。

#### 6.1.1 强引用
当我们新建一个对象时就创建了一个具有强引用的对象，JVM宁愿抛出OOM，使程序异常终止，也不会回收具有强引用的对象来解决内存不足的问题。
#### 6.1.2 软引用
如果一个对象只具有软引用，当内存不足时，会回收这些对象的内存，回收后如果还是没有足够的内存，就会抛出OOM。
#### 6.1.3 弱引用
弱引用比起软引用具有更短的生命周期，垃圾收集器一旦发现了弱引用对象，不管当前内存是否足够，都会回收它的内存。
#### 6.1.4 虚引用
虚引用并不会决定对象的生命周期，如果一个对象仅持有虚引用，这就和没有任何引用一样，在任何时候都可能被垃圾收集器回收。一个只具有虚引用的对象，被垃圾收集器回收时会收到一个系统通知，这也是虚引用的主要作用。

### 6.2 引用计数算法
引用计数算法的基本思想就是每个对象都有一个引用计数器，当对象在某处被引用的时候，它的引用计数器就加1，引用失效就减1.

主流的JVM没有选择引用计数算法来标记垃圾，主要原因是引用计数算法没有解决对象之间循环引用的问题。

### 6.3 根搜索算法
根搜索算法就是选定一些对象作为GC Roots，并组成根对象集合，然后以这些GC Roots的对象作为起始点，向下搜索，如果目标对象到GC Roots是连着的，说明该目标对象是可达的。如果目标对象不可达说明目标对象是可以被回收的对象。
...
### 7.3 不可见阶段
在程序中找不到对象的任何强引用，该对象进入不可见阶段。

### 7.4 不可达阶段
在程序中找不到对象的任何强引用，并且垃圾收集器发现对象不可达。

### 7.5 收集阶段
垃圾收集器已经发现对象的不可达，并且垃圾收集器已经准备好要对该对象的内存空间重新进行分配，这个时候如果该对象重写了finalize方法，则会调用该方法。
...
- 标记和清除的效率都不高
- 容易产生大量不连续的内存碎片，碎片太多会导致后续没有足够的连续内存分配给较大的对象，从而提前触发新的一次垃圾收集动作

### 8.2 复制算法
把内存空间划分为两个相等的区域，每次只使用其中一个区域。在垃圾收集时，遍历当前使用的区域，把存活对象复制到另一个区域中，最后将当前使用的区域可回收的对象进行回收。如果存活对象很少，复制算法效率就会很高，复制算法被广泛应用在**新生代**中。
...
### 8.3 标记-压缩算法
复制算法可以用在新生代中，但是不能用在老年代中，因为老年代的对象存活率高，就会有较多的复制操作，导致效率降低。

标记-压缩算法和标记-清除算法不同的是，在标记可回收的对象后将所有存活的对象压缩到内存的一端，使它们紧凑地排列在一起，然后对边界以外的内存进行回收。回收后，已用和未用的内存都各自一边。

标记-压缩算法解决了标记-清除算法效率低和容易产生大量内存碎片的问题，被广泛用在了**老年代**中。

### 8.4 分代收集算法
分为新生代和老年代垃圾收集。<br>

- Minor Collection：新生代垃圾收集
- Full Collection：老年代进行收集，又可以称为Major Collection。Full Collection通常情况下会伴随至少一次的Minor Collection。它的收集频率较低，耗时较长。

当执行一次Minor Collection时，Eden空间的存活对象会被复制到To Survivor空间，并且之前经过一次Minor Collection并在From Survivor空间存活的仍年轻的对象也会复制到To Survivor空间。有两种情况Eden空间和From Survivor空间存活的对象不会复制到To Survivor空间，而是晋升到老年代。

- 存活的对象的分代年龄超过-XX:Max TenuringThreshold（用于控制对象经历多少次Minor GC才晋升到老年代）所指定的阈值。
- To Survivor空间容量达到阈值。

当所有存活的对象被复制到To Survior空间，或者晋升到老年代，也就意味着Eden空间和From Survivor空间剩下的都是可回收对象。这时GC执行Minor Collection，Eden空间和From Survivor空间就会被清空，新生代中存活的对象都存放在To Survivor空间。接下来将From Survivor空间和To Survivor空间互换位置，每次Survivor空间互换都要保证To Survivor空间是空的，这就是复制算法在新生代中的应用。在老年代中则会采用标记-压缩算法或者标记-清除算法。
